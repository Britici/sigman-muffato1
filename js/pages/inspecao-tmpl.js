// ============================================================
// SIGMAN v2.0 — pages/inspecao-tmpl.js
// ============================================================
// Idêntico a js/pages/preventiva.js — mesmo modelo de dados
// (familias / preventivaTemplates), mesmo fluxo (só imprime, não
// salva execução no app). A ÚNICA diferença é o cabeçalho do PDF
// impresso ("ORDEM DE INSPEÇÃO" em vez de "...PREVENTIVA", doc code
// SIGMAN-INSP). Decisão do Tiago: substituiu a antiga tela "Templates
// de Inspeção" (que tinha o conceito de Rota vs Mecânica/Elétrica,
// removido).
// Fluxo real: PCM cria a O.S. Planejada (tipo Inspeção) em
// Planejamento; manutentor vem aqui, imprime o checklist em branco,
// preenche na mão durante a inspeção, e dá baixa na O.S. Planejada
// (gera a O.S. correlata em Executadas — único registro salvo).
// ============================================================
...
export function init() {
  ...
  if (!_bound) {
    _bound = true;
    document.getElementById('insp-tmpl-sala')?.addEventListener('change', _populateMaquinas);
    document.getElementById('insp-tmpl-maq')?.addEventListener('change', _carregarChecklist);
    document.getElementById('btn-insp-tmpl-print')?.addEventListener('click', _imprimir);
  }
  document.getElementById('insp-tmpl-body').innerHTML = '';
  PLANO_ATUAL = { maquina: null, familia: null, tarefas: [] };
}

function _populateSalas() {
  const db  = getDB();
  const sel = document.getElementById('insp-tmpl-sala');
  if (!sel) return;
  const cur = sel.value;
...
function _populateMaquinas() {
  const db  = getDB();
  const sel = document.getElementById('insp-tmpl-maq');
  if (!sel) return;
  const salaNome = document.getElementById('insp-tmpl-sala')?.value || '';
...
function _carregarChecklist() {
  const db   = getDB();
  const body = document.getElementById('insp-tmpl-body');
  const maqId = document.getElementById('insp-tmpl-maq')?.value;
...
<div class="header">
  ...
  <div><h1>ORDEM DE INSPEÇÃO</h1></div>
  <div style="text-align:right;font-size:11px">Doc: SIGMAN-INSP<br>Rev: 02</div>
</div>
<div class="info-grid">
